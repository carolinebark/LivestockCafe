...
model_ndvi_main_ba <- lm(NDVI ~ after * LC, data = merged_ndvi_chepp_filtered)

glimpse(merged_dataset_ndvi_filtered)
...
```{r}
ndvi_ba <- effect("LC:after", model_ndvi_main_ba)
summary(ndvi_ba)
ndvi_ba <- as.data.frame(ndvi_ba)
ndvi_ba
...
model_soc_main_ba <- lm(SOC ~ after * LC, data = merged_soc_chepp_filtered)
...
model_erosion_main_ba <- lm(Erosion ~ after * LC, data = merged_erosion_chepp_filtered)
