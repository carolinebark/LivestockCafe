...
#   geom_line() +  # Skapa en linjeplot
#   geom_point() + # Lägg till punkter för varje datapunkt
#   scale_color_manual(values = c("1" = "blue", "0" = "red")) + # Ange färger för varje LC
#   theme_minimal() + # Använd ett minimalt tema
...
ggplot(monthly_mean_ndvi, aes(x = Month, y = Mean_NDVI_month, color = factor(LC))) +
  geom_line() +  
  geom_point() + 
  scale_color_manual(values = c("1" = "blue", "0" = "red")) + 
  theme_minimal() + 
...
ggplot(monthly_mean_ndvi_annex, aes(x = Month, y = Mean_NDVI_month_annex, color = factor(LC))) +
  geom_line() +  
  geom_point() + 
  scale_color_manual(values = c("1" = "blue", "0" = "red")) + 
  theme_minimal() + 
...
ggplot(annual_mean_ndvi_annex, aes(x = Year, y = Mean_NDVI_year_annex, color = factor(LC), group = LC)) +
  geom_line() +  
  geom_point() + 
  scale_color_manual(values = c("1" = "blue", "0" = "red")) + 
  theme_minimal() + 
...
ggplot(ls_in_out_melt_lok, aes(x = YearMonth, y = NDVI, fill = factor(LC))) +
  ...
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(title = "Monthly NDVI, Livestock Cafe Lokiriama",
       x = "Month",
       y = "NDVI",
       fill = "LC") + 
...
ggplot(ls_in_out_melt, aes(x = YearMonth, y = NDVI, fill = factor(LC))) +
  geom_boxplot() + 
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
...
ggplot(ls_in_out_melt_annex, aes(x = YearMonth, y = NDVI, fill = factor(LC))) +
  geom_boxplot() + 
  theme_minimal() + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + 
  labs(title = "Monthly NDVI, Annex LC",
       x = "Month",
       y = "NDVI",
       fill = "LC") + 
  scale_fill_manual(values = c("1" = "blue", "0" = "red")) 
...
ls_in_out_melt$LC <- factor(ls_in_out_melt$LC)

ls_in_out_melt <- ls_in_out_melt %>%
  dplyr::select( -`YearMonth_ym`)

model_NDVI_interaction <- lmer(NDVI ~ LC * Time + (1 | Month), data = ls_in_out_melt)
...
#check normality of res manually
main_soc_in_out_long <- main_soc_in_out_long %>%
  dplyr::mutate(E1 = resid(model_soc_interaction),
                fitted = fitted (model_soc_interaction))

# Histogram of the residuals 

hist(main_soc_in_out_long$E1)
main_soc_in_out_long %>%
  ggplot() +
  stat_qq(aes(sample = E1))+
  stat_qq_line(aes(sample = E1))
...
```{r}
ls_in_out_melt_annex$YearMonth_ym <- as.yearmon(format(ls_in_out_melt_annex$Month, "%Y-%m"))
ls_in_out_melt_annex$Time <- as.numeric(ls_in_out_melt_annex$YearMonth_ym - min(ls_in_out_melt_annex$YearMonth_ym))
ls_in_out_melt_annex$LC <- factor(ls_in_out_melt_annex$LC)

ls_in_out_melt_annex <- ls_in_out_melt_annex %>%
  dplyr::select( -`YearMonth_ym`)

model_NDVI_interaction_annex <- lmer(NDVI ~ LC * Time + (1 | Month), data = ls_in_out_melt_annex)
...
performance::check_model(model_NDVI_interaction_annex)
summary(model_NDVI_interaction_annex)
sjPlot::tab_model(model_NDVI_interaction_annex)

plot_NDVI_LC_annex <- plot_model(model_NDVI_interaction_annex, 
                            type = "est",
                            transform = NULL,
                            vline.color = "gray",
                            show.values = TRUE,
                            ...
                            value.size = 3.5,
                            colors = c("#CC6A70", "#22A884"),
                            title = "LC and NDVI over time, Annex")

print(plot_NDVI_LC_annex)


# Predicted values from model, for each year and LC
predicted_NDVI <- ggpredict(model_NDVI_interaction_annex, terms = c("Time", "LC"))
ggplot(predicted_NDVI, aes(x = x, y = predicted, color = group)) +
  geom_line() +
  labs(title = "Predicted NDVI over Time by LC",
       x = "Time since start (years)",
...
```
```{r}
annex_erosion_in_out_long$Year <- as.numeric(as.character(annex_erosion_in_out_long$Year))

annex_erosion_in_out_long$Time <- annex_erosion_in_out_long$Year - min(annex_erosion_in_out_long$Year)
erosion_in_out_long$LC <- factor(erosion_in_out_long$LC)
...
data_filtered_ndvi  <- ls_in_out_melt %>%
  filter(year %in% c(2020, 2023))

data_filtered_ndvi=data_filtered_ndvi %>% mutate(after=if_else(Year>2021, 1, 0))

model_ndvi_main_ba <- lm(NDVI ~ factor(after) * factor(LC), data = data_filtered_ndvi)
...
```{r}
main_soc_in_out_long$Year <- as.numeric(as.character(main_soc_in_out_long$Year))
data_filtered_soc <- main_soc_in_out_long[main_soc_in_out_long$Year %in% c(2020, 2023), ]

data_filtered_soc=data_filtered_soc %>% mutate(after=if_else(year>2021, 1, 0))


model_soc_main_ba <- lm(SOC ~ factor(after) * factor(LC), data = data_filtered_soc)
...
data_filtered_erosion <- erosion_in_out_long %>%
  filter(Year %in% c(2020, 2023))

data_filtered_erosion=data_filtered_erosion %>% mutate(after=if_else(Year>2021, 1, 0))

model_erosion_main_ba <- lm(Erosion ~ factor(after) * factor(LC), data = data_filtered_erosion)
...
soc_diff <- soc_chepp_2023 - soc_chepp_2020
...
plot(cropped_raster, col=colors, main="SOC Change (2023 - 2020) within and around the Polygon", range=c(-15000,15000))
...
erosion_diff <- erosion_chepp_2023 - erosion_chepp_2020
...
plot(cropped_raster_erosion, col=colors, main="Erosion Change (2023 - 2020) within and around the Polygon", range=c(-150,150))
...
plot(cropped_raster_NDVI, col=colors, main="NDVI Change yearly average", range=c(-0.5,0.5))
...
ldsf_combined_spatial <- vect(ldsf_combined_spatial)

livestock_cafe <- terra::extract(rasterLayer, ldsf_combined_spatial, bind = TRUE)

erosion_data <- terra::extract(erosion, ldsf_combined_spatial, bind = TRUE)

soc_chepp_data <- terra::extract(soc_chepp, ldsf_combined_spatial, bind=TRUE)

livestock_cafe_df <- as.data.frame(livestock_cafe_df)
erosion_data_df <- as.data.frame(erosion_data_df)
soc_chepp_data_df <- as.data.frame(soc_chepp_data)

combined_df <- cbind(livestock_cafe_df_df, erosion_data_df_df)
final_combined_df <- cbind(combined_df, soc_chepp_data_df)

unika_kolumner <- names(final_combined_df)[!duplicated(names(final_combined_df))]

filtrerad_df <- final_combined_df[, unika_kolumner]

cleaned_df <- filtrerad_df %>%
  dplyr::select(LC, viserosion1, viserosion2, viserosion3, viserosion4, severeero,
                herb_cov_total, plot, site, cluster, year,
                dplyr::matches("^20"), dplyr::matches("^chep")) %>%
  distinct() %>%
  filter(site == "Chepareria") %>%
  # Lägg till "NDVI" framför kolumnnamn som börjar med "20"
  dplyr::rename_with(~ifelse(grepl("^20", .x), paste0("NDVI ", .x), .x), starts_with("20"))

# Visa de första raderna för att kontrollera ändringarna
head(cleaned_df)

colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_predsoc_2020_StackedModel"] <- "SOC 2020"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_predsoc_2021_StackedModel"] <- "SOC 2021"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_predsoc_2022_StackedModel"] <- "SOC 2022"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_predsoc_2023_StackedModel"] <- "SOC 2023"

colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_severeero_2020_StackedModel"] <- "Erosion 2020"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_severeero_2021_StackedModel"] <- "Erosion 2021"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_severeero_2022_StackedModel"] <- "Erosion 2022"
colnames(cleaned_df)[colnames(cleaned_df) == "chepareria_lc8_s1_pred_severeero_2023_StackedModel"] <- "Erosion 2023"
...
ls_in_out_melt_lok_copy <- dplyr::select(ls_in_out_melt_lok_copy, -c(Month,ID))
ls_in_out_melt_lok_copy$LCID <- 3

ls_in_out_melt_annex_copy <- data.frame(ls_in_out_melt_annex)
ls_in_out_melt_annex_copy <- dplyr::select(ls_in_out_melt_annex_copy, -c(Month,ID,Time))
ls_in_out_melt_annex_copy$LCID <- 2

ls_in_out_melt_copy <- data.frame(ls_in_out_melt)
ls_in_out_melt_copy <- dplyr::select(ls_in_out_melt_copy, -c(Month,ID,Time))
ls_in_out_melt_copy$LCID <- 1

merged_dataset_ndvi <- rbind(ls_in_out_melt_lok_copy, ls_in_out_melt_annex_copy, ls_in_out_melt_copy)

merged_dataset_ndvi <- separate(merged_dataset_ndvi, YearMonth, into = c("Year", "Month"), sep = "-", remove = FALSE, convert = TRUE)
...
main_soc_in_out_long_copy<- dplyr::select(main_soc_in_out_long_copy, -c(E1, fitted,ID,Time))
main_soc_in_out_long_copy$LCID <- 1
...
annex_soc_in_out_long_copy$LCID <- 2
...
lok_soc_in_out_long_copy$LCID <- 3
...
erosion_in_out_long_copy<- dplyr::select(erosion_in_out_long_copy, -c(E1, fitted,ID,Time))
erosion_in_out_long_copy$LCID <- 1

annex_erosion_in_out_long_copy <- data.frame(annex_erosion_in_out_long)
annex_erosion_in_out_long_copy<- dplyr::select(annex_erosion_in_out_long_copy, -c(Time, ID))
annex_erosion_in_out_long_copy$LCID <- 2
...
lok_erosion_in_out_long_copy$LCID <- 3
