...
ggplot(annual_mean_annex_soc, aes(x = Year, y = mean_annex_soc, color = factor(LC), group = LC)) +
  ...
  scale_color_manual(values = c("1" = "blue", "0" = "red")) + 
  theme_minimal() + 
  labs(title = "SOC yearly average, Annex Livestock Cafe",
       x = "Year",
       y = "SOC ",
       color = "LC") 
...
#FLYTTA UPP DENNA DEL OCH ANVÄND DET TILL OVAN OCKSÅ
...
ls_in_out_melt_copy <- dplyr::select(ls_in_out_melt_copy, -c(Month,ID,Time,year))
ls_in_out_melt_copy$LCID <- "B"

ls_in_out_melt_annex_copy <- data.frame(ls_in_out_melt_annex)
ls_in_out_melt_annex_copy <- dplyr::select(ls_in_out_melt_annex_copy, -c(Month,ID))
...
merged_dataset_ndvi$LC <- factor(merged_dataset_ndvi$LC)
...
data_filtered_ndvi <- ls_in_out_melt %>%
  filter(year %in% c(2020, 2023)) %>%
    mutate(after = factor(if_else(year == 2023, 1, 0), levels = c(0, 1)))

model_ndvi_main_ba <- lm(NDVI ~ after * LC, data = data_filtered_ndvi)
...
data_filtered_soc <- main_soc_in_out_long %>%
  filter(Year %in% c(2020, 2023)) %>%
  mutate(after = factor(if_else(Year == 2023, 1, 0), levels = c(0, 1)))

model_soc_main_ba <- lm(SOC ~ after * LC, data = data_filtered_soc)
...
data_filtered_erosion <- data_filtered_erosion %>%
  mutate(after = factor(if_else(Year > 2021, 1, 0)))

model_erosion_main_ba <- lm(Erosion ~ after * LC, data = data_filtered_erosion)
...
soc_chepp_2023_scaled <- soc_chepp_2023 / 1000
soc_chepp_2020_scaled <- soc_chepp_2020 / 1000
...
plot(cropped_raster, col=colors, main="SOC Change (2023 - 2020) within and around the Polygon", range=c(-15,15))
...
erosion_diff <- erosion_chepp_2020 - erosion_chepp_2023  
...
plot(cropped_raster_erosion, col=colors, main="Erosion Change (2023 - 2020) within and around the Polygon", range=c(-100,100))
...
merged_dataset_erosion <- merged_dataset_erosion %>%
  mutate(LC = as.factor(LC),
         LCID = as.factor(LCID),
         after = factor(if_else(Year > 2021, "1", "0")))

model_erosion2 <- lmer(Erosion ~ LC * after + (1 | LCID), data = merged_dataset_erosion)

predicted_erosion <- ggpredict(model_erosion2, terms = c("after", "LC"))

ggplot(predicted_erosion, aes(x = x, y = predicted, color = group)) +
  geom_line() +
  labs(title = "Predicted Erosion over Time by LC",
       x = "Time after Start (Years)",
       y = "Predicted Erosion",
       color = "LC") +
  theme_minimal() +
  scale_color_manual(values = c("1" = "blue", "0" = "red"))
...
plot_erosion_merged <- plot_model(model_erosion2, 
                            type = "est",
                            transform = NULL,
                            vline.color = "gray",
                            show.values = TRUE,
                            ...
                            value.size = 3.5,
                            colors = c("#CC6A70", "#22A884"),
                            title = "LC erosion before and after")

print(plot_erosion_merged)
